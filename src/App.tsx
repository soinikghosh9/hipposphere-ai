...
import { WelcomeScreen } from "./screens/WelcomeScreen"; // Not used by default, but kept for future
...
const AppContent: React.FC = () => {
  ...
  // If on the welcome (front) page, render just FrontPage
  if (isFrontPage) {
    return (
      <Routes>
  ...
  // Otherwise, render the main layout (nav + footer + content)
  return <MainLayout />;
};
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-jungle-green via-earthy-brown to-watery-blue text-gray-100">
      ...
      <nav className="bg-jungle-green/80 backdrop-blur-md shadow-lg p-4 sticky top-0 z-50">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <Link
            to={ROUTES.DASHBOARD}
            className="flex items-center space-x-2 text-2xl font-bold text-accent-teal hover:text-sky-blue transition-colors"
          >
            ...
            {NAV_ITEMS.map((item) => (
              <li key={item.path}>
                <Link
                  to={item.path}
                  className={`
                    px-2 py-2 sm:px-3 text-sm sm:text-base font-medium rounded-md flex items-center space-x-1 transition-all
                    ${
                      location.pathname === item.path
                        ? "bg-accent-teal text-jungle-green"
                        : "text-gray-200 hover:bg-earthy-brown hover:text-white"
                    }
                  `}
                >
      ...
      <main className="flex-grow container mx-auto p-4 sm:p-6 lg:p-8">
        <Routes>
          ...
          {/* Wildcard: if no match, show Dashboard */}
      ...
      <footer className="bg-jungle-green/60 text-center p-4 text-sm border-t-2 border-accent-teal/50">
        ...
        <p className="text-xs mt-1 text-gray-400">
          Stories woven from AI insights & human creativity.
        </p>
      </footer>
    </div>
  );
